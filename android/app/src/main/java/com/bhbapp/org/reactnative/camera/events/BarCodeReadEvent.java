...
public class BarCodeReadEvent extends Event<BarCodeReadEvent> {
  ...
  public static com.bhbapp.org.reactnative.camera.events.BarCodeReadEvent obtain(int viewTag, Result barCode, int width, int height, byte[] compressedImage) {
    com.bhbapp.org.reactnative.camera.events.BarCodeReadEvent event = EVENTS_POOL.acquire();
    if (event == null) {
      event = new com.bhbapp.org.reactnative.camera.events.BarCodeReadEvent();
    }
