...
public class PictureSavedEvent extends Event<PictureSavedEvent> {
    ...
    public static com.bhbapp.org.reactnative.camera.events.PictureSavedEvent obtain(int viewTag, WritableMap response) {
        com.bhbapp.org.reactnative.camera.events.PictureSavedEvent event = EVENTS_POOL.acquire();
        if (event == null) {
            event = new com.bhbapp.org.reactnative.camera.events.PictureSavedEvent();
        }
