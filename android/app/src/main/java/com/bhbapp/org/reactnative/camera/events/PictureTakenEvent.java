...
public class PictureTakenEvent extends Event<PictureTakenEvent> {
  ...
  public static com.bhbapp.org.reactnative.camera.events.PictureTakenEvent obtain(int viewTag) {
    com.bhbapp.org.reactnative.camera.events.PictureTakenEvent event = EVENTS_POOL.acquire();
    if (event == null) {
      event = new com.bhbapp.org.reactnative.camera.events.PictureTakenEvent();
    }
    event.init(viewTag);
    return event;
  }
